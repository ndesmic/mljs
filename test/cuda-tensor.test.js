...
describe("CUDATensor", () => {
	...
	describe("perception example (2x2)", () => {
		it("should forward and backward pass correctly", () => {
			const x = new CUDATensor({ shape: [2, 2], values: [2, 0, 6, 9], label: "x" });
			const w = new CUDATensor({ shape: [2, 2], values: [-3, 1, -1, 2], label: "w" });
			const b = new CUDATensor({ shape: [2], values: [4, 5], label: "b" });

			const xw = x.mul(w, { label: "xw" });
			const i = xw.sum({ dimensionToReduce: 0, label: "i" });
			const t = i.add(b, { label: "t" });
			const o = t.tanh({ label: "o" });
	...
	describe("random", () => {
		it("should give back random values (seeded)", () => {
			...
			const tensor = CUDATensor.random([3, 3], { generator });
